...
- The local server binds only to 127.0.0.1, validates Host and Origin, and requires a random session token in a request header for every API request. The token is given in a URL fragment, removed from the address bar, and stored in localStorage so tabs in the same browser profile share access. It survives browser closure but becomes invalid when the server restarts. The desktop launcher opens the current private URL; a storage event refreshes other connected tabs. No ambient authentication cookie is used. Treat this browser profile as authorized local access until the server stops or its site data is cleared.
- The desktop launcher discovers the current session from `browser-session.json` inside the private data directory and verifies it against the loopback API. The file has owner-only permissions on POSIX; protect the directory with account ACLs on Windows. Session metadata and desktop logs contain bearer credentials and must remain local. Startup metadata is replaced on every server start; stale metadata cannot authorize access.
