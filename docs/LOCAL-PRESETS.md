...
The main views have addresses `/review`, `/monitor`, `/history` and `/settings`. Queue report links open `/history/<report-id>`. Direct navigation and refresh serve the same app shell; API data still requires the session token. Back/Forward follows view navigation. The initial private URL stores the token in localStorage and removes it from the address bar. Bookmarked routes work in new tabs of the connected browser profile. Another browser profile needs its own first connection. The desktop launcher (`mail-sentinel open`, or **Open Mail Sentinel.command** on macOS) reuses the running service or starts it in the background, then opens the current private URL. After a server restart, opening the launcher updates the token in all connected tabs through the browser storage event. It does not overwrite settings or apply a preset. Private session metadata and logs are stored in the data directory; keep this directory out of Git.
