...
In 1.0.0rc2 `finish_investigation` requires a `claims` array with supporting/counter-evidence references and limitations. A non-INCONCLUSIVE finish with obtainable missing checks returns an error with `pending_checks` and keeps the session open for correction. An explicit INCONCLUSIVE handoff is allowed; coverage is reported separately. Native automatic baseline collection belongs to `Agent.run`; MCP clients choose their own tool calls.
