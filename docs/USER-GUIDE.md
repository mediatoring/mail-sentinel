...
**Version 1.0.0rc2 — release candidate awaiting deployment acceptance.** This is not a certified mail gateway or a replacement for existing email security. The included tests verify application behavior and safety boundaries, not phishing detection accuracy.
...
[Step-by-step installation for Windows, macOS and Linux](INSTALL.md) · [Český návod](INSTALL.cs.md)
...
The web UI and queue use API/local-server adapters. [The MCP adapter](MCP.md) exposes single-message evidence tools and guarded completion to external clients such as Codex, Claude Code and Cursor, using the client's own model/login. Direct CLI subprocess providers and direct ChatGPT/Claude web-session authentication are not embedded in the web UI.


## What makes it agentic?

The host first collects required baseline evidence, then supplies a goal and native function tools. On each model iteration, the model selects one tool and arguments. Python validates and executes the call, pseudonymizes the result, assigns an evidence ID, and sends the accumulated evidence back. The model may choose another tool or call `finish_investigation`. Further investigation order is model-selected; baseline checks are host-controlled.
...
Adding Python tools is documented in [docs/EXTENDING.md](EXTENDING.md). Plugins are administrator-installed trusted code, not downloaded by the model.
...
The watcher now uses a persistent SQLite queue, bounded UID discovery and restart recovery. It fetches bodies only when a worker begins processing and never executes quarantine. Configure the received-since date, retries, hourly rate and concurrency in the UI or TOML. The browser queue starts paused after a process restart; `watch` resumes explicitly authorized processing. See [queue operations](QUEUE.md).


See [docs/OPERATIONS.md](OPERATIONS.md) for deployment boundaries and retention.

## Rules, skills and specialists

[Check rules](CHECKS.md) define mandatory, optional and disabled tools. The host emits a completion checklist and prevents LOW_RISK when mandatory evidence is absent, an account mismatches or injection indicators require review. [Runtime skills and scoped specialists](SKILLS-AND-SUBAGENTS.md) are configurable, with shared budgets and no recursive delegation. [Release acceptance](RELEASE-1.0.md) lists implemented capabilities and live-verification limits.
...
See [the security/UX review](REVIEW-0.3.0.md) and [the optional AgentDojo evaluation](AGENTDOJO.md).
...
This project was written independently. Related repositories informed design discussions; no source code from them is included. See [docs/DESIGN.md](DESIGN.md).
...
Start with [the developer guide](DEVELOPMENT.md) and [AGENTS.md](../AGENTS.md). See [changelog](../CHANGELOG.md).
...
See [database configuration](DATA-SOURCES.md), [plugin contract](EXTENDING.md) and [semantic checks](CHECKS.md). Live model language acceptance is available through `python -m evaluation.semantic_eval` and requires configured model access.
...
Set the loaded model context window in Settings to match your model server (`context_tokens`, default 8,192). Input is estimated before each request with room reserved for output; a context failure produces no verdict. Local reasoning models may need a larger context, output budget and timeout. See [model budgets](HARNESS.md#context-and-output-budgets).

Use `python -m sentinel backup backup.sqlite3` for a consistent report/queue snapshot. Configuration, evidence sources and credentials require separate backups. See [operations](OPERATIONS.md) for restoration. Only one server, watcher or scan process can own a data directory.

For deployment acceptance against a real model, run `python -m evaluation.semantic_eval` and `python -m evaluation.demo_eval` from the source checkout. Both use bundled synthetic data and save JSON results; external models additionally require `--allow-external`. Semantic evaluation supports repeatable `--case` selectors and checkpoints progress. See [validation](VALIDATION.md) for measured results and remaining release gates.

Load local mailbox/model presets directly in Settings. Views have their own addresses and support browser Back/Forward and refresh. See [local presets and page addresses](LOCAL-PRESETS.md).
