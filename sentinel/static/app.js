...
function renderMessages(){const box=$('messages');box.replaceChildren();state.messages.forEach(m=>{const b=node('button',undefined,'message'+(selected===m.id?' active':''));b.append(node('span',m.source.toUpperCase(),'badge'),node('strong',m.subject||'(empty subject)'),node('small',m.sender));b.onclick=()=>{if(!running)select(m)};box.append(b)});}
...
function renderJob(j){lastJob=j;$('events').replaceChildren();$('cancelJob').hidden=j.status!=='running';$('jobStatus').textContent=t(j.status==='running'?'working':j.status);for(const ev of j.events||[]){if(ev.type==='tool'){const el=node('div',undefined,'event '+ev.status);el.append(node('strong',ev.id+' · '+ev.tool+(ev.status==='denied'?' · DENIED':'')));const d=node('details');d.append(node('summary',t('observations')),node('pre',JSON.stringify(ev.observation,null,2)));el.append(d);$('events').append(el)}else if(ev.type==='error'){$('events').append(node('p',t(ev.error_code==='context_limit'?'context_limit':'failed'),'notice'));}}if(j.report){const r=j.report,box=$('report');box.replaceChildren();box.hidden=false;box.className='report-card'+(r.verdict==='HIGH_RISK'?' high':'');box.append(node('h3',t(r.verdict)),node('p',r.summary),node('small',t('references')+': '+r.evidence_ids.join(', ')));for(const k of ['uncertainties','recommendations']){box.append(node('h3',t(k)));const ul=node('ul');r[k].forEach(x=>ul.append(node('li',x)));box.append(ul)}}$('approve').hidden=!j.approval_token;$('approve').onclick=()=>safe(async()=>{if(!confirm(t('approveConfirm')))return;await api('approve',{token:j.approval_token,report_id:j.report_id});j.approval_token=null;$('approve').hidden=true;notice(t('moved'));});}
...
async function historyLoad(){const list=await api('reports');$('history').replaceChildren();if(!list.length)$('history').append(node('p',t('noHistory'),'muted'));for(const r of list){const row=node('details');const label=r.result.report?t(r.result.report.verdict):r.result.status;row.append(node('summary',new Date(r.created*1000).toLocaleString(lang==='cs'?'cs-CZ':'en-GB')+' · '+label),node('pre',JSON.stringify(r.result,null,2)));$('history').append(row)}}
...
$('test').onclick=()=>safe(async()=>{$('test').disabled=true;try{await api('connection',{});notice(t('connected'));}finally{$('test').disabled=false}});
$('preview').onclick=()=>safe(async()=>{const p=await api('preview',{message_id:selected});$('payload').textContent=JSON.stringify(p,null,2);$('outgoing').hidden=false;$('outgoing').open=true;previewed=true;});
...
$('settings').onsubmit=e=>{e.preventDefault();safe(async()=>{const data={};for(const el of $('settings').elements){if(el.name&&!el.disabled&&!el.name.startsWith('check_'))data[el.name]=el.type==='checkbox'?el.checked:el.type==='number'?Number(el.value):['enabled_skills','plugins'].includes(el.name)?el.value.split(',').map(x=>x.trim()).filter(Boolean):el.value.trim();}data.check_modes={};for(const el of $('settings').elements){if(el.name?.startsWith('check_'))data.check_modes[el.name.slice(6)]=el.value;}if(!['local','compatible'].includes(data.provider))data.base_url='';if(data.provider==='local')data.allow_external=false;await api('settings',data);$('settings').elements.api_key.value='';$('settings').elements.imap_password.value='';previewed=false;$('consent').checked=false;await refresh();if(selected)$('consentWrap').hidden=!state.external;$('queueConsent').checked=false;notice(t('saved'));});};
