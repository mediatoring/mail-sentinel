...
function renderJob(j){lastJob=j;$('events').replaceChildren();$('cancelJob').hidden=j.status!=='running';$('jobStatus').textContent=t(j.status==='running'?'working':j.status);for(const ev of j.events||[]){if(ev.type==='tool'){const el=node('div',undefined,'event '+ev.status);el.append(node('strong',ev.id+' · '+ev.tool+(ev.status==='denied'?' · DENIED':'')));const d=node('details');d.append(node('summary',t('observations')),node('pre',JSON.stringify(ev.observation,null,2)));el.append(d);$('events').append(el)}else if(ev.type==='error'){$('events').append(node('p',t('failed'),'notice'));}}if(j.report){const r=j.report,box=$('report');box.replaceChildren();box.hidden=false;box.className='report-card'+(r.verdict==='HIGH_RISK'?' high':'');box.append(node('h3',t(r.verdict)),node('p',r.summary),node('small',t('references')+': '+r.evidence_ids.join(', ')));for(const k of ['uncertainties','recommendations']){box.append(node('h3',t(k)));const ul=node('ul');r[k].forEach(x=>ul.append(node('li',x)));box.append(ul)}}$('approve').hidden=!j.approval_token;$('approve').onclick=()=>safe(async()=>{if(!confirm(t('approveConfirm')))return;await api('approve',{token:j.approval_token,report_id:j.report_id});j.approval_token=null;$('approve').hidden=true;notice(t('moved'));});}
