...
if(params.get('token')) {localStorage.setItem('sentinel-token',params.get('token'));sessionStorage.removeItem('sentinel-job');history.replaceState(null,'',location.pathname+location.search);}
// Migrate an already authenticated tab; new tabs share the browser's token.
if(!localStorage.getItem('sentinel-token')&&sessionStorage.getItem('sentinel-token'))localStorage.setItem('sentinel-token',sessionStorage.getItem('sentinel-token'));
sessionStorage.removeItem('sentinel-token');
const token=localStorage.getItem('sentinel-token')||'';
window.addEventListener('storage',event=>{if(event.key==='sentinel-token'&&event.newValue!==token){sessionStorage.removeItem('sentinel-job');location.reload();}});
...
function requireSession(){ $('sessionAccess').hidden=false;$('savedPreset').disabled=true;$('loadPreset').disabled=true;$('presetStatus').textContent=t('presetNeedsSession'); }
async function api(path,data){let r;try{r=await fetch('/api/'+path,{method:data?'POST':'GET',headers:{'X-Sentinel-Token':token,'Content-Type':'application/json'},body:data?JSON.stringify(data):undefined});}catch{throw Error(t('networkFailed'));}const j=await r.json();if(!r.ok){if(r.status===401||r.status===403)requireSession();const e=Error(j.error||'Request failed');e.status=r.status;throw e;}return j;}
...
// Overridden in controls.js once the localized check catalog is available.
function toolLabel(name){return name;}
function toolNote(ev){return ev.status==='denied'?t('tool_denied'):'';}
function renderJob(j){lastJob=j;$('events').replaceChildren();$('cancelJob').hidden=j.status!=='running';$('jobStatus').textContent=t(j.status==='running'?'working':j.status);for(const ev of j.events||[]){if(ev.type==='tool'){const el=node('div',undefined,'event '+ev.status);el.append(node('strong',ev.id+' · '+toolLabel(ev.tool)));const note=toolNote(ev);if(note)el.append(node('p',note,'muted'));const d=node('details');d.append(node('summary',t('observations')),node('pre',JSON.stringify(ev.observation,null,2)));el.append(d);$('events').append(el)}else if(ev.type==='completion_rejected'){$('events').append(node('p',t('completionRetry'),'muted'));}else if(ev.type==='error'){$('events').append(node('p',t(['context_limit','provider_unavailable','output_limit','tool_call_invalid','provider_error'].includes(ev.error_code)?ev.error_code:'failed'),'notice'));}}if(j.report){const r=j.report,box=$('report');box.replaceChildren();box.hidden=false;box.className='report-card'+(r.verdict==='HIGH_RISK'?' high':'');box.append(node('h3',t(r.verdict)),node('p',r.summary),node('small',t('references')+': '+r.evidence_ids.join(', ')));for(const k of ['uncertainties','recommendations']){box.append(node('h3',t(k)));const ul=node('ul');r[k].forEach(x=>ul.append(node('li',x)));box.append(ul)}}$('approve').hidden=!j.approval_token;$('approve').onclick=()=>safe(async()=>{if(!confirm(t('approveConfirm')))return;await api('approve',{token:j.approval_token,report_id:j.report_id});j.approval_token=null;$('approve').hidden=true;notice(t('moved'));});}
